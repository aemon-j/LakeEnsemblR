...
run_MCMC <- function(config_file, num = 100, param_file = NULL, method,
  ...
  .get_init_value <- function(par_range){
    params = as.data.frame(matrix(NA, ncol = nrow(par_range), nrow=1)) # initial parameter guesses
    for (ii in 1:nrow(par_range)){
      var.name = rownames(par_range)
  ...
  .step_param <- function(cur_par, par_range){
    ...
    while (ready){
      df = cur_par[-length(cur_par)] + (runif(len_cur_par,1,2) *runif(len_cur_par,-1,1)/(par_range$ub-par_range$lb)/runif(len_cur_par,1,5))
