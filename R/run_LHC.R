...
run_LHC <- function(config_file, num = NULL, param_file = NULL, method, model = c('FLake', 'GLM', 'GOTM', 'Simstrat'), folder = '.'){
  ...
  obs_file <- get_yaml_value(file = yaml, label = 'observations', key = 'observation_file')
  ...
  if('GOTM' %in% model){
    ...
    got_yaml <- file.path(folder,get_yaml_value(config_file, "config_files", "gotm_config"))
  ...
  fit_file <- gsub('latin_hypercube_params', 'LHS_fit', param_file)
  strat_file <- gsub('latin_hypercube_params', 'LHS_strat', param_file)
