...
run_ensemble <- function(config_file, model = c('GOTM', 'GLM', 'Simstrat', 'FLake', 'MyLake'), folder = '.', return_list = FALSE, create_netcdf = TRUE){
  ...
  if('FLake' %in% model){
    ...
    if(return_list | create_netcdf){
      ...
      if(!is.list(fla_out)){
        ...
        fla_out <- lapply(1:length(fla_out), function(x){
          merge(fla_out[[x]], out_time, by = 1, all.y = T)
        })
  ...
  if('GLM' %in% model){
    ...
    if(return_list | create_netcdf){
      ...
      # Extract output
      glm_out <- get_output(config_file = config_file, model = 'GLM', vars = out_vars, obs_depths = obs_deps, folder = folder)

      # Ensure GLM is on the same time step for output
      if(!is.list(glm_out)){
        glm_out <- merge(glm_out, out_time, by = 'datetime', all.y = T)
      }else{
        glm_out <- lapply(1:length(glm_out), function(x){
          merge(glm_out[[x]], out_time, by = 1, all.y = T)
        })
  ...
  if('GOTM' %in% model){
    ...
    if(return_list | create_netcdf){
      ...
      # Extract output
      gotm_out <- get_output(config_file = config_file, model = 'GOTM', vars = out_vars, obs_depths = obs_deps, folder = folder)

      # Ensure GOTM is on the same time step for output
      if(!is.list(gotm_out)){
        gotm_out <- merge(gotm_out, out_time, by = 'datetime', all.y = T)
      }else{
        gotm_out <- lapply(1:length(gotm_out), function(x){
  ...
  if('Simstrat' %in% model){
    ...
    input_json(sim_par, "Simulation", "Start d", as.numeric(difftime(start_date_simulation, as.POSIXct(paste0(reference_year,"-01-01")), units = "days")))
    input_json(sim_par, "Simulation", "End d", as.numeric(difftime(end_date_simulation, as.POSIXct(paste0(reference_year,"-01-01")), units = "days")))
    ...
    if(return_list | create_netcdf){
      ...
      ### Extract output
      sim_out <- get_output(config_file = config_file, model = 'Simstrat', vars = out_vars, obs_depths = obs_deps, folder = folder)

      # Ensure Simstrat is on the same time step for output
      if(!is.list(sim_out)){
        sim_out <- merge(sim_out, out_time, by = 'datetime', all.y = T)
      }else{
        sim_out <- lapply(1:length(sim_out), function(x){
    ...
    if(create_netcdf){
      ...
      if('temp' %in% out_vars){
        for(i in 1:length(temp_list)){
          ...
          tmp_def <- ncvar_def(paste0(tolower(names(temp_list)[i])), "Celsius", list(lon1, lat2, timedim,depthdim), fillvalue, lname, prec="float", compression = compression, shuffle = FALSE) # Define variable
          nc_vars[[length(nc_vars)+1]] <- tmp_def # Add to list
        }
        names(nc_vars)[(length(nc_vars) - length(temp_list)+1):length(nc_vars)] <- paste0(names(temp_list), '_temp') # Re-assign list names
      }

      if('ice_height' %in% out_vars){
        for(i in 1:length(ice_list)){
          model_name <- strsplit(names(ice_list)[i], '_')[[1]][1]

          lname <- paste(model_name,'Ice height') # Long name
          tmp_def <- ncvar_def(paste0(tolower(names(ice_list)[i])), "m", list(lon1, lat2, timedim), fillvalue, lname, prec="float", compression = compression, shuffle = FALSE) # Define variable
          nc_vars[[length(nc_vars)+1]] <- tmp_def # Add to list
        }
        names(nc_vars)[(length(nc_vars) - length(ice_list)+1):length(nc_vars)] <- paste0(names(ice_list), '_ice_height') # Re-assign list names
      }
