...
#' 
#' Claculates model performace metrics from the netcdf file generated by running `run_ensemble`
#' 
...
#' quality_measures(ncdf = "output/ensemble_output.nc",
...
quality_measures <- function(ncdf, model, var, qualfun = qual_meas) {
 ...
 res_long <- purrr::map(var_long,
                        function(m) dplyr::mutate(m, variable = -as.numeric(gsub("wtr_", "",
                                                                                 variable)))) 

 # calculate quality measures
 qual <- lapply(res_long, function(m){qualfun(m$obs, m$value)})
