...
#' @export
get_output <- function(config_file, model, vars, obs_depths = NULL, folder = '.'){
  ...
  if('FLake' %in% model){
    ...
    fla_out <- read_flake_out(output = file.path(folder, 'FLake', 'output', 'output.dat'), vars = vars, depths = depths, folder = fold, nml_file = nml_file)
