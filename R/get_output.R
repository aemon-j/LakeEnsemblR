...
#' @param folder filepath; to folder which contains the model folders generated by export_config(). Defaults to '.'
...
#' @export
get_output <- function(config_file, model, vars, obs_depths = NULL, folder = '.', out_time, out_hour){
  ...
  if('FLake' %in% model){
    ...
    fla_out <- read_flake_out(output = file.path(folder, 'FLake', 'output', 'output.dat'), vars = vars, depths = depths, folder = fold, nml_file = nml_file, out_time = out_time, out_hour = out_hour)
  ...
  if('GLM' %in% model){
    ...
    if('temp' %in% vars){
      ...
      glm_out[[length(glm_out)+1]] <- get_var(file = file.path(folder, 'GLM', 'output', 'output.nc'), var_name = 'temp', reference = 'surface', z_out = depths)
      colnames(glm_out[[length(glm_out)]]) <- c('datetime',paste('wtr_', depths, sep=""))
      names(glm_out)[length(glm_out)] <- 'temp'
    }

    if('ice_height' %in% vars){
      glm_out[[length(glm_out)+1]] <- get_ice(file = file.path(folder, 'GLM', 'output', 'output.nc'))
  ...
  if('GOTM' %in% model){
    ...
    if('ice_height' %in% vars){
      ice_height <- get_vari(ncdf = file.path(folder, 'GOTM', 'output', 'output.nc'), var = 'Hice', print = FALSE)
      # ice_frazil <- get_vari(ncdf = file.path(folder, 'GOTM', 'output', 'output.nc'), var = 'Hfrazil', print = FALSE)
  ...
  if('Simstrat' %in% model){
    ...
    if('temp' %in% vars){
      ...
      temp <- read.table(file.path(folder, "Simstrat", "output", "T_out.dat"), header = T, sep=",", check.names = F)
    ...
    if('ice_height' %in% vars){
      ice_height <- read.table(file.path(folder, "Simstrat", "output", "TotalIceH_out.dat"), header = T, sep=",", check.names = F)
      ice_height[,1] <- as.POSIXct(ice_height[,1]*3600*24, origin = paste0(reference_year, '-01-01'))
      # In case sub-hourly time steps are used, rounding might be necessary
  ...
  if('MyLake' %in% model){
    ...
    if('temp' %in% vars){
      ...
      temp_interp <- matrix(NA,nrow=length(dates),ncol=length(seq(min(depths),max(depths),by=output_depths)))
      for(i in 1:ncol(temps)){
        temp_interp[i,] <- approx(x=depths,
                                  y=temps[,i],
                                  xout=seq(min(depths),max(depths),by=output_depths),
      ...
      colnames(mylake_out[[length(mylake_out)]]) <- c('datetime',
                                                      paste('wtr_', seq(min(depths),max(depths),by=output_depths), sep=""))
