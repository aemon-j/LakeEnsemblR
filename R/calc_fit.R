...
#' }
calc_fit <- function(ncdf, list = NULL, model, spin_up = 0, var = "temp", dim = "model",
                     dim_index = 1, qualfun = qual_meas, avfun = "mean") {
  ...
  if(spin_up != 0){
    spin_date <- var_list$Obs[1, 1] + spin_up * (24 * 60 * 60)
    var_list <- lapply(var_list, function(x) {
      x[x[, 1] >= spin_date, ]
    })
    if(nrow(var_list[[1]]) == 0) {
      stop("Spin up value is too large. Units are in days, try a smaller value.")
    }
  }
